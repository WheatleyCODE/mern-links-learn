...
router.post('/register', [
    ...
    express_validator_1.check('password', 'Минимальная длинна пароля 6 символов').isLength({ min: 6 })
    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                ...
                _a = req.body // Получаем данные с клиента
                , email = _a.email, password = _a.password;
                return [4 /*yield*/, User_1["default"].findOne({ email: email })
                    // Если нашли пользователя
                ]; // Ищем в базе даннго пользователя
            case 1:
                condidate = _b.sent() // Ищем в базе даннго пользователя
                ;
                // Если нашли пользователя
                if (condidate) {
                    return [2 /*return*/, res.status(400).json({ message: 'Такой пользователь уже существует' })];
                }
